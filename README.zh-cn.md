...
片段应置于返回 error 的启动函数中。导入 `github.com/go-devtools/openapi`，将 `github.com/go-devtools/gin-swagger` 别名设为 `ginswagger`，并导入**自己模块**生成的 `internal/apidoc` 包。完整可编译应用见[基础示例](examples/basic/main.go)。CLI 与应用使用同一固定适配器版本；首次生成允许生成目录尚不存在。
