...
import (
	...
	"github.com/openapi-golang/gin-swagger/internal/integration/testdata/rawrequests"
...
func rawRequestBundle(t *testing.T) openapi.Bundle {
	...
	result, err := core.Compile(context.Background(), core.Options{Load: core.LoadOptions{Dir: "testdata/rawrequests"}, Frontends: []core.Frontend{front.Frontend()}})
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, method := range []string{"GET", "POST", "PUT", "PATCH", "DELETE"} {
		for _, multipartBody := range []bool{false, true} {
			t.Run(method+"/"+map[bool]string{false: "urlencoded", true: "multipart"}[multipartBody], func(t *testing.T) {
				...
				before, after := rawrequests.Router(), rawrequests.Router()
				...
				var actual rawrequests.FormResult
...
func TestRawQueryArray(t *testing.T) {
	...
	for _, target := range []string{"/query?values=first&values=second", "/query"} {
		...
		var result rawrequests.FormResult
...
func TestRawDictionaryGetters(t *testing.T) {
	...
	for _, multipartBody := range []bool{false, true} {
		engine := gin.New()
		engine.POST("/maps", rawrequests.Maps)
		...
		var actual rawrequests.MapResult
...
func TestRawFileSaving(t *testing.T) {
	...
	for _, invalidDestination := range []bool{false, true} {
		...
		for _, mount := range []bool{false, true} {
			...
			engine.POST("/save", rawrequests.Save)
			if mount {
				var err error
...
func TestRawReadDiagnostics(t *testing.T) {
	...
	engine := gin.New()
	engine.GET("/query", rawrequests.Query)
