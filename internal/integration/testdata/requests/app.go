...
type Input struct {
	...
	// 数量。 Count.
	Count int
	// 字节编号而不是 Base64。 Byte identifiers rather than Base64.
	IDs []byte
	// 可选限制。 Optional limit.
	Limit *int
	// 固定两个值。 Exactly two values.
	Pair [2]int
	// 是否启用。 Whether enabled.
	Active bool
	// RFC3339 时间。 RFC3339 timestamp.
	At time.Time
	// Go 持续时间文本。 Go duration text.
	Delay time.Duration
}
