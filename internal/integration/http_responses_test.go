...
import (
	...
	"github.com/openapi-golang/gin-swagger/internal/integration/testdata/httpresponses"
...
func httpResponseBundle(t *testing.T) openapi.Bundle {
	...
	result, err := core.Compile(context.Background(), core.Options{Load: core.LoadOptions{Dir: "testdata/httpresponses"}, Frontends: []core.Frontend{front.Frontend()}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestHTTPRedirects(t *testing.T) {
	...
	for _, method := range []string{"GET", "HEAD", "POST", "PUT", "PATCH", "DELETE"} {
		for _, sample := range []struct {
			...
			{"absolute", httpresponses.RedirectAbsolute, 308, "https://example.test/a/../next?q=%2F", "text/html", true, true},
			{"not-modified", httpresponses.RedirectNotModified, 304, "/cached", "", false, true},
			{"deleted-media", httpresponses.RedirectDeletedMedia, 301, "/next/", "text/html", true, true},
			{"late-media", httpresponses.RedirectLateMedia, 409, "", "", false, false},
			{"whitespace", httpresponses.RedirectWhitespace, 302, "https://example.test/next", "text/html", true, true},
			{"root", httpresponses.RedirectRoot, 302, "/next?from=old", "text/html", true, true},
			{"created", httpresponses.RedirectCreated, 201, "/created", "text/html", true, true},
			{"unicode", httpresponses.RedirectUnicode, 307, "/%e7%9b%ae%e6%a0%87", "text/html", true, true},
			{"relative", httpresponses.RedirectRelative, 303, "/parent/next", "text/html", true, false},
			{"custom", httpresponses.RedirectCustom, 302, "/next", "text/plain", false, true},
			{"committed", httpresponses.RedirectCommitted, 409, "", "text/html", true, false},
			{"dynamic", httpresponses.RedirectDynamic, 302, "/dynamic", "text/html", true, false},
		} {
...
func TestRedirectContinuation(t *testing.T) {
	...
	for _, method := range []string{"POST", "GET"} {
		engine := gin.New()
		engine.Handle(method, "/continue", httpresponses.RedirectContinue)
	...
	engine := gin.New()
	engine.GET("/invalid", httpresponses.RedirectInvalid)
	if _, err := ginswagger.Build(engine, bundle, ginswagger.Config{OpenAPI: openapi.Config{Title: "Invalid", Version: "1"}}); err == nil || !strings.Contains(err.Error(), "重定向状态") {
...
func TestHEADWireResponse(t *testing.T) {
	bundle := httpResponseBundle(t)
	engine := gin.New()
	engine.GET("/head", httpresponses.HeadJSON)
	engine.HEAD("/head", httpresponses.HeadJSON)
