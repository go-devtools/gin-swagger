...
func TestScalarRequestSources(t *testing.T) {
	...
	for _, engine := range engines {
		engine.GET("/scalar/:id", ScalarReads)
		engine.GET("/cookie", CheckedCookie)
	}
		...
		t.Run(sample.name, func(t *testing.T) {
			...
			for i, server := range servers {
				for _, path := range []string{"/scalar/42?" + sample.query, "/cookie"} {
					...
					var actual ScalarResult
					if response.StatusCode != 200 || json.Unmarshal(body, &actual) != nil {
						t.Fatal(response.StatusCode, string(body))
					}
					want := ScalarResult{ID: "42", Query: sample.q, Optional: sample.optional, Present: sample.present, Default: sample.alias, Header: "first", Cookie: sample.value, CookieError: sample.missing}
					if actual != want {
