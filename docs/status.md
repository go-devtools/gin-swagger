...
核心依赖固定为真实远端版本 v0.0.0-20260905193137-a978fefd3c30，Gin 固定 v1.12.0。核心包括独立 Schema 资源作用域导出、显式依赖嵌入、方言与预算，以及响应头提交快照和明确的网络表示 Schema。新的外部 module 对该远端核心版本运行五项公开 SDK race 测试和实际远端安装 CLI 的导出及实例校验，均通过且无 replace。
