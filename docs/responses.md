...
## File serving and explicit project contracts

`File`, `FileAttachment`, and `FileFromFS` delegate to file-serving behavior that depends on the real resource, request preconditions, byte ranges, and filesystem failures. The default frontend reports a diagnostic requiring a complete centralized file contract. A constant path alone does not justify a fixed 200 response, MIME type, length, or automatically invented 206/404 alternatives. Invalid unselected file candidates remain isolated from other routes.

`internal/integration/file_boundaries_test.go` supplies an ordinary public `Frontend.CallOutcomes` extension for a fixed regular UTF-8 text asset. It matches complete Gin method and application-package identities, exact handlers, paths, filesystem root, and attachment name. The profile uses typed neutral response alternatives for text, raw `multipart/byteranges`, and bodyless 304/412; response-header schemas retain attachment and range metadata. Every fact is marked `declared`, and the asset digest and supported methods enter `Options.Configuration`. This is a verified project declaration, not automatic inference for arbitrary files. Unknown assets and request methods outside GET/HEAD retain diagnostics.

The test sends 56 file scenarios to each of two real HTTP servers, before and after mounting documentation: complete content, single/suffix/multiple ranges, invalid or unsatisfiable ranges, conditional 304/412, If-Range fallback, and missing-file 404. GET and HEAD cover all three file entry points. Actual bytes, status, headers, multipart part ranges, and part media types are checked; HEAD body suppression is observed on the HTTP wire. Only server dates and random multipart boundaries are normalized for comparison. Independent schemas validate text and Content-Range values, while the standard multipart reader validates actual range framing.

The profile also declares potential filesystem failure statuses; these tests exercise 404 and range/precondition failures, not every permission or I/O failure behind 403/500. It does not establish directory redirects, symlink policy, arbitrary binary formats, changing assets, or arbitrary request mutation. Such behavior requires its own explicit contract and samples rather than widening this profile silently. Generation leaves the fixture source and asset bytes unchanged.
